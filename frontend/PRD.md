...
1.  **Dashboard Executivo (Home)**:
    *   **Visão Unificada**: Listagem integrada de Projetos e Programas Estratégicos.
    *   **Identificação Visual**: Cartões distintos para Projetos (Cinza) e Programas (Azul) com ícones exclusivos (`FileText` vs `Layers`).
    *   **Filtros Avançados**:
        *   Busca textual (Nome do Projeto/Programa).
        *   Filtros Múltiplos: Diretoria, Product Owner.
        *   Hierarquia de Processos: Macroprocesso, Processo, Subprocesso.
        *   Filtro por Período (Data Início - Fim).
    *   **KPIs Resumidos**: Status, Progresso (%), Qtde. de Fases nos cards.

2.  **Gestão de Projetos (Status Report)**:
    *   Criação e edição de Status Reports semanais.
    *   Monitoramento de prazos, riscos e impedimentos.
    *   Registro de lições aprendidas.
    *   Timeline física do projeto.

3.  **Gestão de Programas Estratégicos (NOVO)**:
    *   Criação e edição de Programas que agrupam múltiplas iniciativas.
    *   **Timeline Editável**: Adição, edição e remoção de Fases e Atividades diretamente na interface.
    *   Cards Estratégicos: Riscos e Lições Aprendidas específicos do programa.
    *   KPIs Consolidados: Visão agregada do progresso do programa.

4.  **Editor de Timeline (NOVO)**:
    *   Interface visual para adicionar/remover Fases.
    *   Gestão de atividades dentro de cada fase (Status, Data, Responsável).
    *   Cálculo automático de progresso baseado nas atividades entregues.
